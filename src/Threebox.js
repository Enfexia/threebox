...
Threebox.prototype = {
  ...
  init: function (map, glContext, options) {
    ...
    this.mapCenterUnits = utils.projectToWorld([
      this.mapCenter.lng,
      this.mapCenter.lat,
    ]);
    ...
    this.enableSelectingFeatures =
      this.options.enableSelectingFeatures || false;
    ...
    this.map.on("load", function () {
      ...
      this.onTap = this.onClick = function (e) {
        ...
        if (intersectionExists) {
          ...
          let nearestObject = Threebox.prototype.findParent3DObject(
            intersects[0]
          );
          if (nearestObject) {
            ...
            let ringSize =
              (hyp / 2) * tb.projectedUnitsPerMeter(map.getCenter().lat) +
              ringRadius;
            ...
            this.ring = tb.Object3D({
              obj: mesh,
              anchor: "center",
              bbox: false,
            });
            this.ring.setCoords([
              ...this.models[nearestObject.userData.id].position,
              0,
            ]);
            this.ring.userData.ring = "hidden";
            this.displayRing = tb.Object3D({
              obj: mesh2,
              anchor: "center",
              bbox: false,
            });
            this.displayRing.setCoords([
              ...this.models[nearestObject.userData.id].position,
              this.tb.ringHeight,
            ]);
      ...
      this.onTouchMove = this.onMouseMove = function (e) {
        ...
        if (
          (this.allowRotate || e.originalEvent.altKey) &&
          this.draggedObject
        ) {
          // this.allowRotate
          ...
          let rotation = {
            ...
            z: Math.round(
              rotationDiff[2] +
                (((~~((current.x - start.x) / this.tb.rotationStep) % 360) *
        ...
        if (this.draggedObject) {
          ...
          tb.world.children.forEach((obj) => {
            ...
            if (obj.userData.ring == "hidden")
              obj.setCoords([options[0], options[1], 0]);
        ...
        if (intersectionExists) {
          ...
          if (
            nearestObject.userData.type != undefined &&
            nearestObject.userData.type != null
          ) {
          ...
          if (nearestObject) {
            ...
            if (
              !this.selectedObject ||
              nearestObject.uuid != this.selectedObject.uuid
            ) {
              if (
                this.overedObject &&
                this.overedObject.uuid != nearestObject.uuid
              ) {
                this.outObject();
              }
          ...
          if (features.length > 0) {
            ...
            if (
              ...
              if (
                ...
                this.selectedFeature.id != features[0].id
              ) {
                this.getCanvasContainer().style.cursor = "pointer";
  ...
  setSunlight: function (newDate, coords) {
    ...
    this.lightLng = this.mapCenter.lng;
    this.lightLat = this.mapCenter.lat;
    this.sunPosition = this.getSunPosition(date, [
      this.mapCenter.lng,
      this.mapCenter.lat,
    ]);
      ...
      this.map.setLight(
        {
          ...
          color: `hsl(40, ${
            50 * Math.cos(this.sunPosition.altitude)
          }%, ${Math.max(20, 20 + 96 * Math.sin(this.sunPosition.altitude))}%)`,
        },
        { duration: 0 }
      );
      if (this.sky) {
  ...
  getSunSky: function (date, sunPos) {
    if (!sunPos) {
      ...
      sunPos = this.getSunPosition(date || Date.now(), [
        center.lng,
        center.lat,
      ]);
    }
  ...
  updateLightHelper: function () {
    if (this.lights.dirLightHelper) {
      this.lights.dirLightHelper.position.setFromMatrixPosition(
        this.lights.dirLight.matrixWorld
      );
  ...
  realSunlight: function (helper = false) {
    ...
    this.lights.dirLight.shadow.camera.top =
      this.lights.dirLight.shadow.camera.right = d2;
    this.lights.dirLight.shadow.camera.bottom =
  ...
  setDefaultView: function (options, defOptions) {
    options.bbox =
      (options.bbox || options.bbox == null) &&
      defOptions.enableSelectingObjects;
    options.tooltip =
      (options.tooltip || options.tooltip == null) && defOptions.enableTooltips;
